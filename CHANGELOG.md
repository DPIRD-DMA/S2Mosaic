...
### Fixed
- **A zero in any band now marks a pixel unusable, rather than only a zero in every band.** `_source_valid_from_bands` asked whether a scene had contributed *anything* at a pixel, which is the right question for a scene footprint and the wrong one for per-band data quality. A pixel where one band had dropped out passed the check on the strength of its other bands, and that band's `0` was copied into the output as if it were a reading. 0 is Sentinel-2 L2A's NODATA and never a measurable reflectance: since baseline 04.00 the `BOA_ADD_OFFSET` of -1000 puts zero reflectance at DN 1000, so DN 0 decodes to -0.1, and before it valid data still started at 1. Observed on Microsoft Planetary Computer at roughly 0.01-0.04% of valid pixels, in dark water, where a single band reads 0 against neighbours near DN 1000 in the same band; Element 84's copy of the same acquisition is unaffected, so this is specific to the product MPC serves. The error is per-band and about 1000 DN where it lands, so it distorts band ratios and spectral indices rather than just darkening a pixel. Scene edges are unchanged in practice: every band drops out together there, and both mask providers already exclude that region, SCL by reporting `NO_DATA` and the OCM path by testing the band sum.
- **Single-band requests are zero-filtered too.** `_source_valid_from_bands` returned `None` for a one-band read, so those mosaics applied no NODATA filtering at all and wrote 0 out as a value. They now go through the same rule as every other request.
