...
- `ocm_inference_dtype` now defaults to `"fp16"` instead of `"bf16"` for broader device support — many GPUs/MPS backends still don't have well-tuned bf16 paths, and fp16 runs everywhere. `"bf16"` is still accepted and is usually marginally faster on hardware that supports it (Ampere+ NVIDIA, Apple Silicon).
