...
itself (via `setuptools-scm`), so there is **no version file to bump**.
...
- **Major** (`v2.0.0` → `v3.0.0`): incompatible API change. Removed args,
  changed signatures, or behavioural changes that break existing callers.
...
a development version like `2.0.1.dev3+g1234abc`, which is useful but not
...
2. **Update [`CHANGELOG.md`](CHANGELOG.md)**: add a `## [<version>] - <YYYY-MM-DD>`
   ...
   `### Added` / `### Changed` / `### Fixed`. There is no `[Unreleased]` block; entries
   are written straight under the version being released. Commit on `main`:
   ```bash
...
3. **Tag and push**:
   ...
   `v2.0.0b3` and `v2.0.0rc1` both match, as did the earlier `v2.0.0b1` and
   `v2.0.0b2` betas.
...
4. **Approve the deploy**: the [`Publish to PyPI`](.github/workflows/publish.yml)
   ...
   The `pypi` GitHub environment is gated by a required reviewer, so go to
